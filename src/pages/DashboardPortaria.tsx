...
function DashboardPortariaTV() {
  ...
  // Garantir que os dados zereiem exatamente às 00:00:00 (meia-noite)
  useEffect(() => {
    ...
    const scheduleMidnight = () => {
      ...
      // Timeout para o próximo midnight
      const t = setTimeout(() => {
        // Forçar refetch imediato à meia-noite
        refetchCotton?.();
        refetchCargas?.();
        refetchGestaoTempo?.();
        refetchVehicles?.();
        ...
        // Depois agendar interval de 24h para as próximas meia-noites
    ...
    const cancel = scheduleMidnight();
    return () => {
      clearAll();
      if (typeof cancel === 'function') cancel();
    };
  }, [refetchCotton, refetchCargas, refetchGestaoTempo, refetchVehicles, refetchLoadings, refetchMaterials, refetchEquipment]);
  ...
  // Debug: mostrar contagem de cargas e primeiros registros
  console.log('DEBUG: cargas count =', cargas?.length);
              ...
              <Card className={`backdrop-blur-lg border transition-all duration-300 ${
                ...
                <CardHeader className={`border-b p-[clamp(0.5rem,1vw,1rem)] transition-colors duration-300 ${
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
                    ...
                    {/* Totais por placa agora ficam dentro de cada card (canto superior direito) */}
                      ...
                      {rankingDiaArray.length > 0 ? (
                        <div className="mt-2 grid grid-cols-1 gap-3">
                          {rankingDiaArray
                            ...
                            .filter(r => (r.rolos || 0) > 0) // remover placas sem rolos
                            .map((r) => {
                              // filtrar apenas viagens do dia para esta placa e com rolos > 0
                              // deduplicar por viagem_num e preparar arrays: primeiras 3 + extras
                              const plateCargasAll = (() => {
                                const items = (cargas || [])
                                  .filter(c => c.placa === r.plate && (c.qtd_rolos || 0) > 0)
                                  .sort((a,b) => (a.viagem_num || 0) - (b.viagem_num || 0));
                                const map = new Map<number, typeof items[0]>();
                                for (const it of items) {
                                  const num = Number(it.viagem_num) || 0;
                                  if (!map.has(num)) map.set(num, it);
                                }
                                return Array.from(map.values());
                              })();
                              const plateCargas = plateCargasAll.slice(0, 3);
                              // Agrupa em colunas de 3: col0 = viagens 1-3, col1 = 4-6, col2 = 7-9, ...
                              const plateCargasColumns: Array<typeof plateCargasAll> = [];
                              for (let i = 0; i < plateCargasAll.length; i += 3) {
                                  ...
                                  <div className="flex items-center justify-between">
                                    <div className="min-w-0">
                                      <div className="flex items-baseline gap-3">
                                        <span className="text-lg font-bold text-white truncate">{r.plate}</span>
                                        <span className="text-sm text-emerald-300 truncate">{r.driver}</span>
                                      </div>
                                        {/* per-plate totals: mostrar total de rolos e viagens no canto superior direito do card por placa */}
                                        <div className="absolute right-2 top-2 text-right">
                                          <div className="text-xs text-muted-foreground">Total</div>
                                          <div className="font-bold text-white text-sm">{(r.rolos || 0).toLocaleString('pt-BR')} R</div>
                                          <div className="text-[0.75rem] text-emerald-300">{r.viagens}v</div>
                                        </div>
                                    </div>
                                  </div>

                                  <div className="mt-2 flex items-start gap-3 text-[0.9rem]">
                                    {plateCargasColumns && plateCargasColumns.length > 0 ? (
                                      plateCargasColumns.map((col, colIndex) => (
                                        <div key={`col-${colIndex}`} className="flex flex-col space-y-1 min-w-[120px]">
                                          {col.map(pc => (
                                            <div key={`${pc.placa}-${pc.viagem_num}`} className="flex items-center justify-between">
                                              <div className="flex items-center gap-2 min-w-0">
                                                <div className="text-sm font-semibold text-emerald-300 w-6 text-center">{pc.viagem_num}</div>
                                                <div className="min-w-0">
                                                  <div className="flex items-center gap-3">
                                                    <div className="flex items-center gap-1">
                                                      <Leaf className="w-4 h-4 text-cyan-300" />
                                                      <span className="text-xs text-cyan-200">
                                                        {pc.viagem_num === 1 ? formatTime(pc.tempo_lavoura || 0) : (pc.tempo_lavoura && pc.tempo_lavoura > 0 ? formatTime(pc.tempo_lavoura) : '-')}
                                                      </span>
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                                      <Factory className="w-4 h-4 text-yellow-300" />
                                                      <span className="text-xs text-yellow-200">{pc.tempo_algodoeira && pc.tempo_algodoeira > 0 ? formatTime(pc.tempo_algodoeira) : '-'}</span>
                                                    </div>
                                                  </div>
                                                </div>
                                              </div>
                                      ...
                                      <div className="text-emerald-400">Sem detalhe de viagens hoje</div>
                                    )}
                                  </div>
                                </div>
                              );
                        ...
                        <p className="text-emerald-400">Nenhum movimento hoje</p>
                      )}
                    </div>
                  </div>
                </CardContent>
